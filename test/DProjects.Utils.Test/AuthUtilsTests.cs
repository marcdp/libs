...
namespace DProjects.Utils.Tests {
    ...
    public class AuthUtilsTests {
        ...
        public void CreateBasicTest(string user, string pass, string result) {
            Assert.Equal(result, DProjects.Utils.AuthHttpBasicUtils.CreateHeader(new NetworkCredential(user, pass)));
        }
        ...
        //    Assert.Equal(result, DProjects.Utils.AuthHttpHmacUtils.CreateHeader(new NetworkCredential(user, pass), method, path, query, contentType, dateHeader != null ? DateTime.Parse(dateHeader) : default, dateHeaderToUse != null ? DateTime.Parse(dateHeaderToUse) : default));
        //}
    }
