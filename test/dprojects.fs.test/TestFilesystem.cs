...
namespace DProjects.Fs.Test {
    ...
    public class FilesystemFactoryByUrl : IFactoryByUrl<IFilesystem> {
        private ServiceProvider mServiceProvider;
        public FilesystemFactoryByUrl() {
            var services = new ServiceCollection();
            services.AddFactoryByUrl<IFilesystem>((services) => {
                services.AddFactoriesFromAssembly<DProjects.Fs.Assembly>();
            });
            mServiceProvider = services.BuildServiceProvider();
        }
        public IFilesystem Create(string url) {
            var fsFactory = mServiceProvider.GetService<IFactoryByUrl<IFilesystem>>()!;
            return fsFactory.Create(url);
        }
    }



    public abstract class TestFilesystem : IDisposable {


        //vars
        protected FilesystemFactoryByUrl mFilesystemFactoryByUrl;
        ...
        //constructor
        public TestFilesystem(string url) {
            mFilesystemFactoryByUrl = new FilesystemFactoryByUrl();
