...
namespace DProjects.XShell.Services.XTemplate {
    ...
    internal static partial class XTemplateTransformers {
        ...
        public static object? Apply(string name, object input, IReadOnlyList<object?> arguments, string? locale, int offset) {
            return name switch {
                ...
                "date" => Date(input, arguments, GetCulture(locale, offset), offset),
                "datetime" => DateTime(input, arguments, GetCulture(locale, offset), offset),
                "time" => Time(input, arguments, GetCulture(locale, offset), offset),
                _ => throw Error($"Unknown transformer '{name}'", offset)
            };
        }
        ...
        private static string Percent(object input, IReadOnlyList<object?> arguments, CultureInfo culture, int offset) {
            ...
            if (!double.IsFinite(percentage)) throw Error("Transformer 'percent' produced a non-finite intermediate result", offset);
        ...
        private static string Currency(object input, IReadOnlyList<object?> arguments, CultureInfo culture, int offset) {
            ...
            if (arguments[0] is not string code || !CurrencyCode().IsMatch(code) || !Currencies.TryGetValue(code, out var currency)) throw Error("Transformer 'currency' requires a supported uppercase ISO 4217 currency code", offset);
        ...
        private static string String(object input, IReadOnlyList<object?> arguments, CultureInfo culture, int offset, bool upper) {
            ...
            if (input is not string text) throw Error($"Transformer '{(upper ? "upper" : "lower")}' requires a string input", offset);
        ...
        private static bool Contains(object input, IReadOnlyList<object?> arguments, int offset) => StringPredicate(input, arguments, "contains", (text, value) => text.Contains(value, StringComparison.Ordinal), offset);
        ...
        private static string FormatNumber(double number, int? digits, CultureInfo culture, int offset) {
            ...
            catch (FormatException) { throw Error("Unsupported transformer precision", offset); }
        }
        ...
        private static DateTimeOffset ParseDate(object input, int offset, bool dateAllowed) {
            ...
            throw Error("Transformer received an invalid ISO-8601 value", offset);
        }
        private static string FormatPattern(DateTimeOffset value, string pattern, CultureInfo culture, bool dateAllowed, bool timeAllowed, int offset) {
            ...
            for (var position = 0; position < pattern.Length;) {
                ...
                if (token == null) {
                    if (char.IsLetter(pattern[position])) throw Error("Transformer pattern contains an unsupported token", offset);
                    result.Append(pattern[position++]);
                    continue;
                }
                if (!dateAllowed && token is "yyyy" or "MMMM" or "MMM" or "MM" or "M" or "dd" or "d") throw Error("Transformer pattern token is not allowed", offset);
                if (!timeAllowed && token is "HH" or "H" or "mm" or "ss") throw Error("Transformer pattern token is not allowed", offset);
                result.Append(token switch {
                    ...
                    "HH" => value.Hour.ToString("D2", CultureInfo.InvariantCulture), "H" => value.Hour.ToString(CultureInfo.InvariantCulture), "mm" => value.Minute.ToString("D2", CultureInfo.InvariantCulture), "ss" => value.Second.ToString("D2", CultureInfo.InvariantCulture), _ => throw Error("Transformer pattern contains an unsupported token", offset)
                });
                hasToken = true;
                position += token.Length;
            }
            if (!hasToken) throw Error("Transformer pattern must contain a token", offset);
        ...
        private static string Pattern(IReadOnlyList<object?> arguments, string name, int offset) {
            ...
            return arguments[0] as string ?? throw Error($"Transformer '{name}' requires a string pattern", offset);
        }
        private static bool StringPredicate(object input, IReadOnlyList<object?> arguments, string name, Func<string, string, bool> predicate, int offset) {
            RequireCount(name, arguments, 1, 1, offset);
            if (input is not string text) throw Error($"Transformer '{name}' requires a string input", offset);
            if (arguments[0] is not string value) throw Error($"Transformer '{name}' requires a string argument", offset);
            return predicate(text, value);
        }
        private static int Digits(object? value, int offset) {
            if (value is not double number || number < 0 || number != Math.Truncate(number) || number > MaximumDigits) throw Error("Transformer digits must be a supported non-negative integer", offset);
            return (int)number;
        }
        private static double InputNumber(object input, string name, int offset) => input is double number && double.IsFinite(number) ? number : throw Error($"Transformer '{name}' requires a numeric input", offset);
        private static void RequireCount(string name, IReadOnlyList<object?> values, int minimum, int maximum, int offset) { if (values.Count < minimum || values.Count > maximum) throw Error($"Transformer '{name}' received an invalid argument count", offset); }
        private static CultureInfo GetCulture(string? locale, int offset) { if (string.IsNullOrWhiteSpace(locale)) return CultureInfo.InvariantCulture; try { return CultureInfo.GetCultureInfo(locale); } catch (CultureNotFoundException) { throw Error($"Unsupported transformer locale '{locale}'", offset); } }
