    ...
    public static class ConsoleUtils {
        ...
        public static string ColorizeLog(string log) {
            ...
            for (var l = 0; l < lines.Length; l++) {
                ...
                result.Append(COLOR_BRBLACK).Append(line, tsStart, i - tsStart).Append(ATTRIBUTES_NONE);
                // skip space
                ...
                if (i < length && line[i] == '[') {
                    ...
                    if (levelInner.StartsWith("ERR", System.StringComparison.OrdinalIgnoreCase)) { 
                        ...
                        levelColor = COLOR_BRBLACK;
                    } else {
                        levelColor = COLOR_BRGREEN; // Information
                    }
                    result.Append(levelColor).Append(levelToken).Append(ATTRIBUTES_NONE);
                    // skip space
                    if (i < length && line[i] == ' ') { result.Append(' '); i++; }
                }
                // message: rest of the line
                if (i < length) {
                    result.Append(COLOR_WHITE).Append(line, i, length - i).Append(ATTRIBUTES_NONE);
                }
            }
            return result.ToString();
        }
